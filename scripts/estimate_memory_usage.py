...
parser = argparse.ArgumentParser(description='Estimate memory needed to run H2O for a given dataset.')
...
string_size = 128
string_requirement = string_size * args.n_num_cols * args.nrows

uuid_size = 16
uuid_requirement = uuid_size * args.n_num_cols * args.nrows

cat_size = 2
cat_requirement = cat_size * args.n_num_cols * args.nrows

time_size = 8
time_requirement = time_size * args.n_num_cols * args.nrows
...
mem_req = BASE_MEM_REQUIREMENT_MB + (data_requirement * SAFETY_FACTOR / BYTES_IN_MB)

mem_req_rounded = math.ceil(mem_req)

print(mem_req_rounded, end="MB\n")
