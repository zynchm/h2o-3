...
class RadixOrder extends H2O.H2OCountedCompleter<RadixOrder> {
  ...
  public void compute2() {
    ...
    Log.info("Time to use rollup stats to determine biggestBit: " + ((t1=System.nanoTime()) - t0) / 1e9+" seconds."); t0=t1;
    ...
    Log.info("Time of MSB count MRTask left local on each node (no reduce): " + ((t1=System.nanoTime()) - t0) / 1e9+" seconds."); t0=t1;
    ...
    Log.info("SplitByMSBLocal MRTask (all local per node, no network) took : " + ((t1=System.nanoTime()) - t0) / 1e9+" seconds."); t0=t1;

    if( _whichCols.length > 0 )
      new SendSplitMSB(linkTwoMRTask).doAllNodes();
    Log.info("SendSplitMSB across all nodes took : " + ((t1=System.nanoTime()) - t0) / 1e9+" seconds."); t0=t1;
    ...
    Log.info("took : " + ((t1=System.nanoTime()) - t0) / 1e9); t0=t1;

    Log.info("Waiting for RPC SingleThreadRadixOrder to finish ... ");
    for( RPC rpc : radixOrders )
      rpc.get();
    Log.info("took " + (System.nanoTime() - t0) / 1e9+" seconds.");
