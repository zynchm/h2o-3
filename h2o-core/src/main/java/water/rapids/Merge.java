...
public class Merge {
  ...
  public static Frame merge(final Frame leftFrame, final Frame riteFrame, final int leftCols[], final int riteCols[],
    ...
    for (int i=0; i<id_maps.length; i++) { // id_maps start with leftFrame levels first followed by rite frame.
      if (id_maps[i] == null) continue;    // id_maps -1 represent riteFrame levels not matched to left frame
