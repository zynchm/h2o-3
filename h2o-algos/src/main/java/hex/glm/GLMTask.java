 ...
 static public class YMUTask extends MRTask<YMUTask> {
  ...
  static abstract class GLMGradientTask extends MRTask<GLMGradientTask> {
    ...
    private final void computeNumericEtas(Chunk [] chks, double [] etas, double [] vals, int [] ids) {
      final int nNumCols = _dinfo._nums;
      final int numIndexStart = _dinfo.numStart();
      if (nNumCols > 0) { // only do anything if there are numerical columns
        int betaIndex = numIndexStart;
        int trueCid = 0;  // take care of enum by num interactions
        for (int cid = 0; cid < nNumCols; ++cid) {  // already taken care of expanding interaction column from enum x num
          trueCid = betaIndex-numIndexStart;
          final double scale = _dinfo._normMul != null ? _dinfo._normMul[trueCid] : 1;
          final double off = _dinfo._normSub != null ? _dinfo._normSub[trueCid] : 0;
          final double NA = _dinfo._numNAFill[cid];
          int chkIndex = cid + _dinfo._cats;
          Chunk c = chks[chkIndex];
          if (c.isSparseZero()) {
            int nvals = c.getSparseDoubles(vals, ids, NA);
            betaIndex = updateEtas(chks, chkIndex, etas, vals, off, _beta, betaIndex, 
                    _dinfo.isInteractionVec(chkIndex), scale, numIndexStart);
    ...
    /*
     * This method calculates the eta = transpose(beta)*X+beta0.  However, note that beta includes the expanded
     * numerical columns generated by enum by num interactions.  Things can get tricky here.  When there are enum by
     * num interactions, the beta and number of columns in chks no longer track each other.
     */
    public int updateEtas(Chunk[] chks, int cid, double[] etas, double[] vals, double off, double[] beta, 
                          int betaIndex, boolean isInteraction, double scale, int numIndexStart) {
      boolean treatAsNum = false;
      InteractionWrappedVec.InteractionWrappedChunk cInt = isInteraction?
              (InteractionWrappedVec.InteractionWrappedChunk) chks[cid]:null;
      int betaIncrement = 1;  // by default, increment by 1
      if (isInteraction) {
        if (!cInt._c1IsCat && !cInt._c2IsCat) { // num by num interaction
          treatAsNum = true;
        } else {  // dealing with enum by num or num by enum interactions
          boolean useAllFactors = ((InteractionWrappedVec)chks[cid].vec())._useAllFactorLevels;
          betaIncrement = cInt.vec().domain().length - (useAllFactors?0:1);
        } 
      } else {
          treatAsNum = true;
      }
      if (treatAsNum) { // nothing special about column
        double b = scale * beta[betaIndex];
        for (int i = 0; i < vals.length; ++i)
          etas[i] += (vals[i] - off) * b;
        
      } else {  // dealing with enum x num interaction columns here
        double predVal = 0.0;
        for (int i = 0; i < vals.length; ++i) { // add in contributions of each predictor
          int betaOff = _dinfo.getInteractionOffset(chks, cid, i);
          for (int enumColInd = 0; enumColInd < betaIncrement; enumColInd++) {  // add effect of each enum x num column
            predVal = betaOff == enumColInd ? vals[i] : 0.0;
            int trueBetaIndex = betaIndex + enumColInd;
            int trueCid = trueBetaIndex - numIndexStart;
            scale = _dinfo._normMul != null ? _dinfo._normMul[trueCid] : 1;
            off = _dinfo._normSub != null ? _dinfo._normSub[trueCid] : 0;
            double b = scale * beta[trueBetaIndex]; // keep the order of calculation as before
            etas[i] += (predVal - off) * b;
          }
        }
      }
      return (betaIndex+betaIncrement);
    }

    public int updateGrad(Chunk[] chks, int cid, double[] gradient, double[] etas, double[] vals, double off,
                          int gradIndex, boolean isInteraction, int valsLen, double scale, int numIndexStart) {
      int gradIncrement = 1;  // default to 1
      boolean treatAsNum = false;
      InteractionWrappedVec.InteractionWrappedChunk cInt = isInteraction?
              (InteractionWrappedVec.InteractionWrappedChunk) chks[cid]:null;
      if (isInteraction) {
        if (!cInt._c1IsCat && !cInt._c2IsCat) { // num by num interaction
          ...
          boolean useAllFactors = ((InteractionWrappedVec)chks[cid].vec())._useAllFactorLevels;
          gradIncrement = cInt.vec().domain().length - (useAllFactors?0:1);
        }
      } else {
        treatAsNum = true;
      }
      if (treatAsNum) { // keep the same order of operation for data frames with no interaction columns
        double g=0;
        for (int i=0; i<valsLen;i++) {
          g += (vals[i] - off) * scale * etas[i];
        }
        gradient[gradIndex] = g;
      } else {  // deal with enum x num interaction
        double predVal = 0;
        for (int i = 0; i < valsLen; i++) {
          int gradOff = treatAsNum ? 0 : _dinfo.getInteractionOffset(chks, cid, i);
          for (int enumColInd = 0; enumColInd < gradIncrement; enumColInd++) {
            int trueGradIndex = gradIndex + enumColInd;
            int trueCid = trueGradIndex - numIndexStart;
            predVal = enumColInd == gradOff ? vals[i] : 0.0;
            scale = _dinfo._normMul != null ? _dinfo._normMul[trueCid] : 1;
            off = _dinfo._normSub != null ? _dinfo._normSub[trueCid] : 0;
            gradient[trueGradIndex] += (predVal - off) * scale * etas[i];
          }
        }
      }
      return (gradIncrement+gradIndex);
    }

    private final void computeNumericGrads(Chunk [] chks, double [] etas, double [] vals, int [] ids) {
      final int numCols = _dinfo._nums;
      if (numCols > 0) {
        final int numColStart = _dinfo.numStart();
        int gradInd = numColStart; // index into expanded cols
        for (int cid = 0; cid < numCols; ++cid) { // go through all numeric columns expanded with enum x num interaction
          double NA = _dinfo._numNAFill[cid];
          int chunkIndex = cid + _dinfo._cats;
          Chunk c = chks[chunkIndex];
          int trueCid = gradInd - numColStart;
          double scale = _dinfo._normMul == null ? 1 : _dinfo._normMul[trueCid];
          double offset = _dinfo._normSub == null ? 0 : _dinfo._normSub[trueCid];
          if (c.isSparseZero()) {
            int nVals = c.getSparseDoubles(vals, ids, NA);
            gradInd = updateGrad(chks, chunkIndex, _gradient, etas, vals, offset, gradInd,
                    _dinfo.isInteractionVec(chunkIndex), nVals, scale, numColStart);
          } else if (c.isSparseNA()) {
            double off = _dinfo._normSub == null ? 0 : _dinfo._normSub[trueCid];
            int nVals = c.getSparseDoubles(vals, ids, NA);
            gradInd = updateGrad(chks, chunkIndex, _gradient, etas, vals, off, gradInd,
                    _dinfo.isInteractionVec(chunkIndex), nVals, scale, numColStart);
          } else {
            double off = _dinfo._normSub == null ? 0 : _dinfo._normSub[trueCid];
            c.getDoubles(vals, 0, vals.length, NA);
            gradInd = updateGrad(chks, chunkIndex, _gradient, etas, vals, off, gradInd, 
                    _dinfo.isInteractionVec(chunkIndex), vals.length, scale, numColStart);
          }
        }
      }
    }
    ...
    public void map(Chunk [] chks) {
      ...
      double [] vals = MemoryManager.malloc8d(response._len); // store response value from whole chunk
      ...
      if(_dinfo._normSub != null) {
        ...
        for(int i = 0; i < _dinfo._nums; ++i) { // todo: fix this for interaction
          if(chks[_dinfo._cats+i].isSparseZero()) {
